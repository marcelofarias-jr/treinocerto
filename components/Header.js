...
export default function Header() {
  ...
  return (
    <>
      ...
      {/* Barra de descanso — aparece em todas as páginas quando o timer de descanso está ativo */}
      {restRemaining > 0 && !isTrainoAtivo && (
        <Link href="/treino"
          className="md:hidden fixed left-0 right-0 z-40 flex items-center justify-between px-4 py-2.5 bg-zinc-800 border-t border-zinc-700"
          style={{ bottom: hasActiveWorkout ? '96px' : '64px' }}
        >
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold text-zinc-300 uppercase tracking-widest">Descanso</span>
          </div>
          <span className="font-heading font-black text-white text-base tracking-widest">{fmt(restRemaining)}</span>
        </Link>
      )}
